...
function initCanBeOpened(showButton, showKey, preventOpenKey, contextKey) {
  eGm.showButton = showButton;
  eGm.showKey = showKey;
  eGm.preventOpenKey = preventOpenKey;
...
QUnit.test("test canBeOpened with left button", function(assert) {
  ...
  assert.ok(eGm.canBeOpened(0, false, false, false));
  assert.notOk(eGm.canBeOpened(1, false, false, false));
  assert.notOk(eGm.canBeOpened(0, true, false, false));
  assert.notOk(eGm.canBeOpened(0, false, true, false));
  assert.notOk(eGm.canBeOpened(0, true, true, false));
  assert.ok(eGm.canBeOpened(0, false, false, true));
  assert.notOk(eGm.canBeOpened(0, true, false, true));
  assert.notOk(eGm.canBeOpened(0, false, true, true));
  assert.notOk(eGm.canBeOpened(0, true, true, true));
});
...
QUnit.test("test canBeOpened with left button, prevent with alt key", function(assert) {
  ...
  assert.notOk(eGm.canBeOpened(0, false, true, false));
  assert.notOk(eGm.canBeOpened(0, true, true, false));
  assert.notOk(eGm.canBeOpened(0, false, false, true));
  assert.notOk(eGm.canBeOpened(0, true, false, true));
  assert.notOk(eGm.canBeOpened(0, false, true, true));
  assert.notOk(eGm.canBeOpened(0, true, true, true));
});
...
QUnit.test("test canBeOpened with left button and shift key", function(assert) {
  ...
  assert.notOk(eGm.canBeOpened(0, false, false, false));
  assert.notOk(eGm.canBeOpened(1, false, false, false));
  assert.ok(eGm.canBeOpened(0, true, false, false));
  assert.notOk(eGm.canBeOpened(0, false, true, false));
  assert.notOk(eGm.canBeOpened(0, true, true, false));
  assert.notOk(eGm.canBeOpened(0, false, false, true));
  assert.ok(eGm.canBeOpened(0, true, false, true));
  assert.notOk(eGm.canBeOpened(0, false, true, true));
  assert.notOk(eGm.canBeOpened(0, true, true, true));
});
...
QUnit.test("test canBeOpened with middle button", function(assert) {
  ...
  assert.ok(eGm.canBeOpened(1, false, false, false));
  assert.notOk(eGm.canBeOpened(0, false, false, false));
  assert.notOk(eGm.canBeOpened(1, true, false, false));
  assert.notOk(eGm.canBeOpened(1, false, true, false));
  assert.notOk(eGm.canBeOpened(1, true, true, false));
  assert.ok(eGm.canBeOpened(1, false, false, true));
  assert.notOk(eGm.canBeOpened(1, true, false, true));
  assert.notOk(eGm.canBeOpened(1, false, true, true));
  assert.notOk(eGm.canBeOpened(1, true, true, true));
});
...
QUnit.test("test canBeOpened with middle button, context with ctrl key", function(assert) {
  ...
  assert.notOk(eGm.canBeOpened(0, false, false, false));
  assert.notOk(eGm.canBeOpened(1, true, false, false));
  assert.ok(eGm.canBeOpened(1, false, true, false));
  assert.notOk(eGm.canBeOpened(1, true, true, false));
  assert.ok(eGm.canBeOpened(1, false, false, true));
  assert.notOk(eGm.canBeOpened(1, true, false, true));
  assert.ok(eGm.canBeOpened(1, false, true, true));
  assert.notOk(eGm.canBeOpened(1, true, true, true));
});
...
QUnit.test("test canBeOpened with middle button, prevent with alt key", function(assert) {
  ...
  assert.notOk(eGm.canBeOpened(1, false, true, false));
  assert.notOk(eGm.canBeOpened(1, true, true, false));
  assert.notOk(eGm.canBeOpened(1, false, false, true));
  assert.notOk(eGm.canBeOpened(1, true, false, true));
  assert.notOk(eGm.canBeOpened(1, false, true, true));
  assert.notOk(eGm.canBeOpened(1, true, true, true));
});
...
QUnit.test("test canBeOpened with middle button and shift key", function(assert) {
  ...
  assert.notOk(eGm.canBeOpened(1, false, false, false));
  assert.notOk(eGm.canBeOpened(0, false, false, false));
  assert.ok(eGm.canBeOpened(1, true, false, false));
  assert.notOk(eGm.canBeOpened(1, false, true, false));
  assert.notOk(eGm.canBeOpened(1, true, true, false));
  assert.notOk(eGm.canBeOpened(1, false, false, true));
  assert.ok(eGm.canBeOpened(1, true, false, true));
  assert.notOk(eGm.canBeOpened(1, false, true, true));
  assert.notOk(eGm.canBeOpened(1, true, true, true));
});
...
QUnit.test("test canBeOpened with middle button and shift key, context with ctrl key", function(assert) {
  ...
  assert.ok(eGm.canBeOpened(1, true, false, false));
  assert.notOk(eGm.canBeOpened(1, false, true, false));
  assert.ok(eGm.canBeOpened(1, true, true, false));
  assert.notOk(eGm.canBeOpened(1, false, false, true));
  assert.ok(eGm.canBeOpened(1, true, false, true));
  assert.notOk(eGm.canBeOpened(1, false, true, true));
  assert.ok(eGm.canBeOpened(1, true, true, true));
});
...
QUnit.test("test canBeOpened with right button", function(assert) {
  ...
  assert.ok(eGm.canBeOpened(2, false, false, false));
  assert.notOk(eGm.canBeOpened(0, false, false, false));
  assert.notOk(eGm.canBeOpened(2, true, false, false));
  assert.notOk(eGm.canBeOpened(2, false, true, false));
  assert.notOk(eGm.canBeOpened(2, true, true, false));
  assert.ok(eGm.canBeOpened(2, false, false, true));
  assert.notOk(eGm.canBeOpened(2, true, false, true));
  assert.notOk(eGm.canBeOpened(2, false, true, true));
  assert.notOk(eGm.canBeOpened(2, true, true, true));
});
...
QUnit.test("test canBeOpened with right button, context with ctrl key", function(assert) {
  ...
  assert.notOk(eGm.canBeOpened(0, false, false, false));
  assert.notOk(eGm.canBeOpened(2, true, false, false));
  assert.ok(eGm.canBeOpened(2, false, true, false));
  assert.notOk(eGm.canBeOpened(2, true, true, false));
  assert.ok(eGm.canBeOpened(2, false, false, true));
  assert.notOk(eGm.canBeOpened(2, true, false, true));
  assert.ok(eGm.canBeOpened(2, false, true, true));
  assert.notOk(eGm.canBeOpened(2, true, true, true));
});
...
QUnit.test("test canBeOpened with right button, prevent with alt key", function(assert) {
  ...
  assert.notOk(eGm.canBeOpened(2, false, true, false));
  assert.notOk(eGm.canBeOpened(2, true, true, false));
  assert.notOk(eGm.canBeOpened(2, false, false, true));
  assert.notOk(eGm.canBeOpened(2, true, false, true));
  assert.notOk(eGm.canBeOpened(2, false, true, true));
  assert.notOk(eGm.canBeOpened(2, true, true, true));
});
...
QUnit.test("test canBeOpened with right button and shift key", function(assert) {
  ...
  assert.notOk(eGm.canBeOpened(2, false, false, false));
  assert.notOk(eGm.canBeOpened(0, false, false, false));
  assert.ok(eGm.canBeOpened(2, true, false, false));
  assert.notOk(eGm.canBeOpened(2, false, true, false));
  assert.notOk(eGm.canBeOpened(2, true, true, false));
  assert.notOk(eGm.canBeOpened(2, false, false, true));
  assert.ok(eGm.canBeOpened(2, true, false, true));
  assert.notOk(eGm.canBeOpened(2, false, true, true));
  assert.notOk(eGm.canBeOpened(2, true, true, true));
});
...
QUnit.test("test canBeOpened with right button and shift key, context with ctrl key", function(assert) {
  ...
  assert.ok(eGm.canBeOpened(2, true, false, false));
  assert.notOk(eGm.canBeOpened(2, false, true, false));
  assert.ok(eGm.canBeOpened(2, true, true, false));
  assert.notOk(eGm.canBeOpened(2, false, false, true));
  assert.ok(eGm.canBeOpened(2, true, false, true));
  assert.notOk(eGm.canBeOpened(2, false, true, true));
  assert.ok(eGm.canBeOpened(2, true, true, true));
});
