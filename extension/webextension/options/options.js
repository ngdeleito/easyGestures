...
/* global window, browser, document, alert, confirm */

const DEFAULT_FAVICON_URL = "defaultFavicon.svg";

function preOptionsLoadHandler() {
  // ugly hack to allow enough time to retrieve the localized action names
  setTimeout(optionsLoadHandler, 500);
}

window.addEventListener("load", preOptionsLoadHandler);
...
browser.runtime.sendMessage({
  messageName: "query_eGActions_getLocalizedActionNames",
}).then(aMessage => {
  aMessage.response.forEach(localizedTuple => {
    eGActions[localizedTuple[0]].getLocalizedActionName = localizedTuple[1];
  });
});
