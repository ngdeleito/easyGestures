...
function eG_menuLayout (menu, name, actionsPrefs, labelsPrefs) {
	...
	for (var i=0; i<actionsPrefs.length; i++) {
		if ( !this.isLarge && (i==3 || i==7) ) {}	// don't push actions that are intended for large menus
			...
			if (eG_menuItems[actionsPrefs[i]].type==1) {
				if (menu.dropDownMenu) eG_menuItems[actionsPrefs[i]].src="moreDropDownMenu";
				else eG_menuItems[actionsPrefs[i]].src="more";
			}
	...
	for (var i = 0; i<this.actions.length; i++){
		if (this.update[this.actions[i].src]==-1) this.update[this.actions[i].src]=i;
	}
	...
	if (!this.isExtraMenu ) this.menuImage += menu.largeMenu ? "largeMenu.png": "menu.png";
	else this.menuImage += "extraMenu.png";		// extra menus are never large
	
	this.tooltipsImage=menu.skinPath;
	if (!this.isExtraMenu ) this.tooltipsImage += (this.hasExtraMenuAction ? "other_": "")+(menu.largeMenu ? "largeLabels.png":"labels.png");
	else this.tooltipsImage+= "extraLabels.png";
	...
	// labels positioning
	this.xLabelsPos= this.isLarge  ? (menu.smallIcons ? new Array(10, 212, 219, 219, 211, 186, 10,10, 10, 10 ) : new Array(10, 290, 300, 300, 285, 230, 10, 10, 10, 10 ) )
							: (menu.smallIcons ? new Array(10, 190, 195, 190, 167, 10, 10, 10): new Array(10, 265, 270, 260, 230, 10, 10, 10) );
	this.yLabelsPos= this.isLarge  ? (menu.smallIcons ? new Array(6, 23, 41, 60, 79, 97, 80, 62, 42, 25) : new Array(10, 40, 70, 100, 130, 162, 130, 100, 70, 40) )
							: (menu.smallIcons ? new Array(6, 23, 41, 60, 77, 58, 42, 25) : new Array(10, 40, 70, 100, 125, 95, 68, 40 ) );
...
function eG_menu () {
	...
	this.menuSet = {    // contains main, extra, alternatives and contextual menu layouts objects
		main:					new eG_menuLayout(this, "main",					prefs.getCharPref("actions.main").split("/"),				    prefs.getComplexValue("actions.labels.main" , Components.interfaces.nsISupportsString).data.split("•")),
		mainAlt1:				new eG_menuLayout(this, "mainAlt1",				prefs.getCharPref("actions.mainAlt1").split("/"), 			    prefs.getComplexValue("actions.labels.mainAlt1", Components.interfaces.nsISupportsString).data.split("•") ),
		mainAlt2:				new eG_menuLayout(this, "mainAlt2",				prefs.getCharPref("actions.mainAlt2").split("/"), 			    prefs.getComplexValue("actions.labels.mainAlt2", Components.interfaces.nsISupportsString).data.split("•") ),
		extra:				    new eG_menuLayout(this, "extra",					prefs.getCharPref("actions.extra").split("/"),				    prefs.getComplexValue("actions.labels.extra", Components.interfaces.nsISupportsString).data.split("•") ),
		extraAlt1:				new eG_menuLayout(this, "extraAlt1",				prefs.getCharPref("actions.extraAlt1").split("/"), 			prefs.getComplexValue("actions.labels.extraAlt1", Components.interfaces.nsISupportsString).data.split("•") ),
		extraAlt2:				new eG_menuLayout(this, "extraAlt2",				prefs.getCharPref("actions.extraAlt2").split("/"), 			prefs.getComplexValue("actions.labels.extraAlt2", Components.interfaces.nsISupportsString).data.split("•") ),
		contextLink:			new eG_menuLayout(this, "contextLink",				prefs.getCharPref("actions.contextLink").split("/"), 			prefs.getComplexValue("actions.labels.contextLink", Components.interfaces.nsISupportsString).data.split("•") ),
		contextImage:			new eG_menuLayout(this, "contextImage",			prefs.getCharPref("actions.contextImage").split("/"), 			prefs.getComplexValue("actions.labels.contextImage", Components.interfaces.nsISupportsString).data.split("•") ),
		contextSelection:		new eG_menuLayout(this, "contextSelection", 		prefs.getCharPref("actions.contextSelection").split("/"), 		prefs.getComplexValue("actions.labels.contextSelection", Components.interfaces.nsISupportsString).data.split("•") ),
		contextTextbox:			new eG_menuLayout(this, "contextTextbox", 			prefs.getCharPref("actions.contextTextbox").split("/"), 		prefs.getComplexValue("actions.labels.contextTextbox", Components.interfaces.nsISupportsString).data.split("•") ),
	}	
...
eG_menu.prototype = {
	...
	createSpecialNodes : function (layoutName) {	//creating DOM nodes
        ...
        if (this.dropDownMenu) {
            ...
            img.style.top=(layout.hasExtraMenuAction? this.lineHeight+this.headerHeight+ this.iconSize/2:- this.iconSize/2)  +"px";
        } else {
		...
		img.setAttribute ("onclick", "var matchCase = this.src.search('matchCase_On')!=-1; this.src=this.src.replace( (matchCase?'On':'Off') , (matchCase?'Off':'On'));this.previousSibling.focus();");
		img.style.visibility = "hidden";
        ...
        for (var i=1;i<=6;i++){
			...
			if (query !="" && chksearch) {
                ...
                if (chksearch) searchEnginesCount++;
	...
	createActionsNodes : function (layoutName) {	//creating DOM nodes
		...
		for(var i = 0; i<nbItems; i++) {	
			...
			if (layout.actions[i].src.search("loadURLScript")==-1 && layout.actions[i].src.search("runProgramFile")==-1) {
				...
				if (layout.actions[i].type==1) this.extraMenuAction = i;
			}
				...
				else {timg.setAttribute("class", ( (this.smallIcons ? "small_":"") + (this.noIcons ? "empty": layout.actions[i].src) ) );}
			}
	...
	createDropDownNodes : function (layoutName) {
		...
		for (var i=0; i<nbItems; i++) {
			...
			if (layout.isExtraMenu) lineNode.style.backgroundColor=eGc.lightgray;
			
			// set default selection
			if (i==0 && !layout.hasExtraMenuAction &&  !layout.isExtraMenu || i==1 && layout.hasExtraMenuAction)  lineNode.style.backgroundImage= "url('chrome://easygestures/skin/lineSelection.png')";
			...
			if (layout.actions[i].type!=1)  tspan.appendChild(eGc.frame_doc.createTextNode(layout.labels[i]));		// don't display Extra Menu label with drop down menu not to confuse user
	...
	show : function (layoutName) {	// makes menu visible
		...
		if (this.showButton==2) {	// right click: make sure "Disable or replace context menus" Browser option is checked so that eG can control right click context menu display
			if (eGc.allowContextBrowserOptionTimerId !=null ) clearTimeout(eGc.allowContextBrowserOptionTimerId);
			var pref = Components.classes["@mozilla.org/preferences-service;1"].getService(Components.interfaces.nsIPrefBranch); 
			eGc.allowContextBrowserOption = pref.getBoolPref("dom.event.contextmenu.enabled");
			if (!eGc.allowContextBrowserOption) pref.setBoolPref("dom.event.contextmenu.enabled",true);
		}

        // create resources if necessary        
        
        if (this.popup==null || document.getElementById("eG_popup")==null) this.createPopup();
        
        var existingNode = eGc.frame_doc.getElementById("eG_SpecialNodes");
	    if (this.specialNodes==null || existingNode ==null) this.createSpecialNodes("main");
		...
		if (this.dropDownMenu) {
			...
			if (layout.aNode == null || existingNode == null) this.createDropDownNodes(layoutName);	// checking if menu has already been displayed in the current document
			else if (layout.aNode != existingNode ) layout.aNode = existingNode;
			...
			if (this.menuState == 0) this.menuState = 1;	// menu is showing
			...
			// create resources if necessary
            var existingNode = eGc.frame_doc.getElementById("eG_actions_"+layoutName);
			if (layout.aNode == null ||  existingNode == null) this.createActionsNodes(layoutName);	// checking if menu has already been displayed in the current document
			else if (layout.aNode != existingNode ) layout.aNode = existingNode;
			...
			if (!layout.isExtraMenu) this.specialNodes.style.top=this.clientY + layout.aNodeYOff+ "px";
	...
	handleMousemove : function (event) {	// handle rollover effects and switch to/from extra menu
		...
		if (this.menuState == 1 && (Math.abs(event.clientX- this.clientX)> 1 || Math.abs(event.clientY- this.clientY)> 1)) this.menuState= 2;
		...
		var moveAutoTrigger;
		if (this.dropDownMenu) {
			if (layout.isExtraMenu) nbItems=5;
		...
		else moveAutoTrigger =(radius>= layout.outerR && layout.actions[sector].type!=1);
		...
		if (this.dropDownMenu) {
			...
			if (dy<-this.lineHeight) i = -1;
			else if (dy < 0 ) i=0
				else i = Math.floor( dy / this.lineHeight ) + 1;
			...
			if (i>=0 && i<nbItems) {    // with extra menu, 7 nodes are created but only 5 are visible. So be carefull with indexes.
                ...
                var cur=i;
                if (layout.isExtraMenu && (cur>2 && cur<5) ) cur=cur+3;   
                
				if (layout.actions[cur].type==1) layout.aNode.childNodes[cur].firstChild.setAttribute("active", "true");
				else {
                ...
                var top = i-1;
				if (top>=0) {
                    if (layout.isExtraMenu && (top>2 && top<5) ) top=top+3;
                ...
                var bottom = i+1;
				if (bottom <nbItems) {
                    if (layout.isExtraMenu && (bottom>2 && bottom<5) ) bottom=bottom+3 ;
			...
			this.sector=i;
			if (layout.isExtraMenu && i> 2) this.sector+=3;
						
			if (dy < - 3*this.lineHeight || dy > nbItems*this.lineHeight || dx> (layout.width + this.iconSize) || dx< -this.iconSize)  { 
				if (this.menuState!=2) this.close();
				else {
					this.menuState=3;	// this trick will trigger the close function next move
					this.sector=-1;
				}
			}
			else if (layout.hasExtraMenuAction && dy <-this.lineHeight) {
				
				this.showExtraMenu();

			} else if (bottomMenu && layout.isExtraMenu) { // hide extra menu
		...
		if (this.sector!= sector) {	// moved to new sector
			...
			if (sector>= 0) {	// sector targetted exists: highlighting icons and labels
				...
				if (layout.lNode!=null) layout.lNode.childNodes[sector].style.fontWeight= "bold";
				...
				if ( this.menuState == 2 && (layout.actions[sector].src.search("prevTab")>=0 || layout.actions[sector].src.search("nextTab")>=0) && layout.aNode.childNodes[sector].getAttribute("grayed")!="true") {
					this.tabPopupTimerId = setTimeout("eGm.tabPopupTimerFlag=true", this.tabPopupDelay);
				}
		...
		if (radius> 3*layout.outerR)  {	if (this.menuState!=2) this.close();    }
	...
	showExtraMenu : function () {
		...
		if (this.dropDownMenu) {
			...
			var baseLayout = this.menuSet[this.baseMenu];
			if (baseLayout.lNode!=null) baseLayout.lNode.style.display="none";
		}
	},

	hide : function (layout) {	// makes menu invisible
		
		if (layout.aNode!=null ) layout.aNode.style.display="none";
		if (layout.lNode!=null ) layout.lNode.style.display="none";
		...
		if (this.showTooltips) clearTimeout(this.tooltipsTrigger);
	...
	clearRollover : function (layout, hidding) {	// clear rollover effect
        ...
        if (this.sector>= 0) {
            if (this.dropDownMenu) {
                ...
                if (layout.lNode!=null) layout.lNode.childNodes[this.sector].style.fontWeight= "normal";
            }
        }
    ...
    rolloverExternalIcons : function (src, node, active) {    // this is for runProgramFile and loadURLScript actions icons which can be customized or imported from favicons or program's icons
        try {
            if (active) {
                if (this[src][4]=="true") node.style.backgroundPosition=(!this.smallIcons?"9px 1px":"1px 1px");
                else if (this[src][5]=="true") node.style.backgroundPosition=(!this.smallIcons?"1px 1px":"1px 1px");
            } else {
                if (this[src][4]=="true") node.style.backgroundPosition=(!this.smallIcons?"8px 0px":"0px 0px");
                else if (this[src][5]=="true") node.style.backgroundPosition=(!this.smallIcons?"0px 0px":"0px 0px");            
            }
        } catch(ex){}
    },
	...
	runAction : function () {
		...
		if (this.sector>=0) {
			...
			if ( layout.name.search("extra") ==-1) {	// main
				...
				if (!layout.isLarge) {
					if (this.sector>2) sector8To10++;
					if (this.sector>=6) sector8To10++;
				}
			...
			} catch(ex) { alert("Exception: "+ ex.toString()); }

            if (this.popup!=null && this.popup.state == 'showing') return; // return to avoid closing menu too early when eG_popup is visible
		}

		if (this.sector==-1 || layout.actions[this.sector].type!=1 ) this.close(); 	// close menu if no extra menu is called from action or no action
	...
	update : function () {	// update menu content (gray actions, display special signs etc..)			
		...
		if (eGc.link != null && this.handleLinks && this.menuState!=2 && this.menuState!=3 && this.curLayoutName =="main") 	{ //if a link is pointed and mouse not dragged
			...
			this.linkTrigger = setTimeout("eGm.linkSign.style.visibility='hidden';", this.linksDelay);
		}
		else this.linkSign.style.visibility="hidden";
		...
		if (layout.update[actionName]!=-1) {
			...
			if (getWebNavigation().canGoBack) actionNode.setAttribute("grayed", "false");
			else actionNode.setAttribute("grayed", "true");
		}
		...
		if (layout.update[actionName]!=-1) {
			actionNode=!this.dropDownMenu ? layout.aNode.childNodes[layout.update[actionName] ]: layout.aNode.childNodes[layout.update[actionName] ].firstChild;
			if (getWebNavigation().canGoForward) actionNode.setAttribute("grayed", "false");
			else 	actionNode.setAttribute("grayed", "true");
		}
		...
		if (layout.update[actionName]!=-1) {
			actionNode=!this.dropDownMenu ? layout.aNode.childNodes[layout.update[actionName] ]: layout.aNode.childNodes[layout.update[actionName] ].firstChild;
			if (getBrowser().mTabContainer.childNodes.length> 1) actionNode.setAttribute("grayed", "false");
			else 	actionNode.setAttribute("grayed", "true");
		}
		...
		if (layout.update[actionName]!=-1) {
			actionNode=!this.dropDownMenu ? layout.aNode.childNodes[layout.update[actionName] ]: layout.aNode.childNodes[layout.update[actionName] ].firstChild;
			if (Cc["@mozilla.org/browser/sessionstore;1"].getService(Ci.nsISessionStore).getClosedTabCount(window) > 0) actionNode.setAttribute("grayed", "false");
			else 	actionNode.setAttribute("grayed", "true");
		}
		...
		if (layout.update[actionName]!=-1) {
			...
			if (winEnum.hasMoreElements() ) winEnum.getNext();		//first window		
			
			if (winEnum.hasMoreElements() ) actionNode.setAttribute("grayed", "false");
			else actionNode.setAttribute("grayed", "true");
		}
		...
		if (layout.update[actionName]!=-1) {
			...
			if (!eGc.loading ) actionNode.setAttribute("class", actionClass.replace("stop","reload") );
			else 	actionNode.setAttribute("class", actionClass.replace("reload","stop"));
		}

		actionName="up";
		if (layout.update[actionName]!=-1) {
			actionNode=!this.dropDownMenu ? layout.aNode.childNodes[layout.update[actionName] ]: layout.aNode.childNodes[layout.update[actionName] ].firstChild;
			if (eG_canGoUp() ) actionNode.setAttribute("grayed", "false");
			else 	actionNode.setAttribute("grayed", "true");
		}
	...
	switchLayout : function () {    // this is not about switching to/from extra menu
		...
		if (this.curLayoutName.search("context") ==-1) {	//switch to alternative layouts
			...
			if (!layout.isExtraMenu)  {
				nextLayoutName ="main";
				if (this.curLayoutName=="main"  && this.mainAlternative1) nextLayoutName= "mainAlt1";
				else if (this.curLayoutName=="mainAlt1"  && this.mainAlternative2 || this.curLayoutName=="main" && !this.mainAlternative1 && this.mainAlternative2) nextLayoutName= "mainAlt2";
			}
			else {
				nextLayoutName ="extra";
				if (this.curLayoutName=="extra"  && this.extraAlternative1) nextLayoutName= "extraAlt1";
				else if (this.curLayoutName=="extraAlt1"  && this.extraAlternative2 || this.curLayoutName=="extra" && !this.extraAlternative1 && this.extraAlternative2) nextLayoutName= "extraAlt2";				
			}
			...
			if (eGc.contextType.split("/").length>2) {
				...
				this.hide(layout);
				if (eGc.contextType=="link/image/")  nextLayoutName = (this.curLayoutName=="contextLink" ? "contextImage" : "contextLink" );
				else nextLayoutName = (this.curLayoutName=="contextSelection" ? "contextTextbox" : "contextSelection" );
	...
	close : function () {
		...
		this.hide(layout);
		if (layout.isExtraMenu) this.hide(baseLayout);  // hide base menu too if closing is done from extra menu
	...
	resetTooltipsTimeout : function () {	// setting and resetting tooltips timeout
		...
		if (this.showTooltips){
			...
			if (this.showingTooltips || this.tooltipsDelayOmit) this.showMenuTooltips();
			else this.tooltipsTrigger = setTimeout("eGm.showMenuTooltips();", this.tooltipsDelay);
		}
	...
	showInputBox : function (enterURL, showInputBoxSignForHighlight) { // showInputBoxSignForHighlight is to display options sign for highlight or SearchWeb actions
		...
		if (this.showTooltips && !this.showingTooltips) clearTimeout(this.tooltipsTrigger);
		...
		if (enterURL) {	// URL input
			...
			if (eGc.selection!="") this.inputBox.value = eGc.selection;
			else this.inputBox.value = window._content.location.href;
		}
		else	{// text input
			...
			if (!this.dropDownMenu) {
				...
				if (layout.isExtraMenu && sector> 5) sector-=3;
	...
	showMenuTooltips : function () {	// displaying tooltips
		...
		if (this.dropDownMenu) {		
			...
			// create resources if necessary
			if (layout.lNode == null || eGc.frame_doc.getElementById("eG_labels_"+this.curLayoutName) == null) this.createLabelsNodes(this.curLayoutName); 	 // checking if labels have already been displayed in the current document
			else if (layout.lNode != eGc.frame_doc.getElementById("eG_labels_"+this.curLayoutName) ) layout.lNode = eGc.frame_doc.getElementById("eG_labels_"+this.curLayoutName);
    ...
    showPopupForSearchWeb : function (closeMenuAndRunSearchOnHiding) {
        ...
        while(this.popup.hasChildNodes()){ this.popup.removeChild(this.popup.firstChild);}   // remove all from menupopup
        if (closeMenuAndRunSearchOnHiding) this.popup.addEventListener("popuphiding", eG_popup, true);
		...
		for (var i=1; i<=6; i++) {
            ...
            if (i==1) itemNode.setAttribute("default", true);
            ...
            if (query !="" && chksearch) {
                ...
                if (closeMenuAndRunSearchOnHiding) itemNode.setAttribute("oncommand", "eGm.inputBoxSignForSearchWeb.setAttribute('currentEngine'," +i+ " );eGm.runAction();");
                else itemNode.setAttribute("oncommand", "eGm.inputBoxSignForSearchWeb.src = this.getAttribute('src');eGm.inputBoxSignForSearchWeb.setAttribute('currentEngine'," +i+ " );");
                try {
                    faviconPath = faviconService.getFaviconForPage(ios.newURI(ios.newURI(query, null, null).prePath,null,null)).spec;
                } catch(e){
                    faviconPath = ios.newURI(query, null, null).prePath + "/favicon.ico";
        ...
        if (searchEnginesCount>1)  {
            ...
            if (closeMenuAndRunSearchOnHiding) itemNode.setAttribute("oncommand", "eGm.inputBoxSignForSearchWeb.setAttribute('currentEngine',0);eGm.runAction();");
            else itemNode.setAttribute("oncommand", "eGm.inputBoxSignForSearchWeb.src = this.getAttribute('src');eGm.inputBoxSignForSearchWeb.setAttribute('currentEngine',0);");
        ...
        if (eGm.showTooltips) clearTimeout(eGm.tooltipsTrigger);
    },
