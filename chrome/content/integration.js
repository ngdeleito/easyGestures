...
if (window.document.location !='chrome://easygestures/content/options.xul') {// avoid a call when options dialog is opened or closed
...
function eG_initMenuIntegration(evt) {	// set the prefs, initiate menu
	...
	if (eG_prefsObs.prefs.getBoolPref("profile.statusbarShowIcon")) {
		...
		statusbarpanel= document.createElementNS("http://www.mozilla.org/keymaster/gatekeeper/there.is.only.xul", "statusbarpanel");
	...
	if (active) {
		...
		skinPath=eG_prefsObs.prefs.getCharPref("skin.path");
        ...
        if(!sss.sheetRegistered(u, sss.AGENT_SHEET)) sss.loadAndRegisterSheet(u, sss.AGENT_SHEET);
		
		if (window.arguments[0]=="about:blank") loadURI("about:blank");	// FIX TRICK: when homepage is set to Blank Page, icons won't show because of CSS unless loading explicitly the blank page...
		...
		if(!wenum.hasMoreElements() ) { // only one Firefox window is open
			if(eGm.startupTips) window.openDialog('chrome://easygestures/content/tips.xul', '', 'chrome,centerscreen,resizable');	// show tip dialog
		}	
	}
}
...
function eG_updatePrefs(prefs) {
	try{
		...
		if (prevVersion==eGc.version) { // no new version
			...
			else{ // update all preferences for all versions prior to version 4.1.2
...
function eG_activateMenu() {
	...
	if (contextMenu) contextMenu.addEventListener("popupshowing", eG_handlePopup, true);
...
function eG_changeStatus() {
	...
	if (active) eG_activateMenu();
		...
		var contextMenu= document.getElementById("contentAreaContextMenu");
		if (contextMenu) contextMenu.removeEventListener("popupshowing", eG_handlePopup, true);	
	}
}
...
function eG_handleKeys(evt) {
	...
	if (eGm.showTooltips && !eGm.showingTooltips) clearTimeout(eGm.tooltipsTrigger);
			
	if (evt.type=="keyup") {eGc.keyPressed=0; }
	else eGc.keyPressed=evt.keyCode;

	if (evt.keyCode == 18 && eGm.menuState!=0 && evt.type=="keydown") {			// <Alt> key is pressed (use right key)
        eGm.switchLayout();
        return;
	}

	// show textarea for typing and retrieve typed text as selected text
	if(eGm.menuState!=0 && evt.keyCode != eGm.contextKey && evt.keyCode != eGm.showKey && evt.keyCode != eGm.supprKey) {    // all keys except keys to control display of pie
		if (evt.keyCode == 13 || evt.keyCode == 27) evt.preventDefault();
		
		if (evt.type=="keydown" ){

				if (eGm.inputBox.style.visibility!="visible" && evt.keyCode != 27 && !evt.shiftKey ){		// Any key will unhide the inputBox
					eGm.showInputBox(true, false);	// URL entry only
				}
				else if  (evt.keyCode == 13 || evt.keyCode == 27)  { // <Enter> key pressed

					eGc.selection = eGm.inputBox.value;
					
					if (eGm.sector!=-1) eGc.lastTypedWord = eGm.inputBox.value;	// remember the last typed word in inputBox except addresses
					eGm.inputBox.blur();					// removing the cursor
	
					if (evt.keyCode != 27) { // any other key except <Escape> key pressed
						if (eGc.selection != "" ) { //do nothing if selection is empty
							if (eGc.selection == "/") {	// open options dialog if character "/" (slash) typed
								window.openDialog("chrome://easygestures/content/options.xul","","");
							}
							else {
								if (eGm.sector == -1) {
									var postData = { };
									if (evt.shiftKey) {	// load url in new tab if <Shift+Enter> pressed
									...
									else	// load url in current tab if only <Enter> pressed
...
function eG_handleMouseup(evt)	{
	...
	if (document.getElementById("content").mCurrentBrowser._scrollingView ==null) {
		if (eGm.autoscrolling) {
			...
			if (eGm.menuState == 0) return;		// avoid contextual menu when autoscrolling ends (this would be triggered below)
		}
	}
	
	if (eGm.typingText) { evt.preventDefault(); return false; }
	...
	if (eGm.menuState== 0) {
		if (!eGm.autoscrolling){	// avoid enabling contextual menu when autoscrolling 
			eGc.blockStdContextMenu = false;
		...
		if (eGm.menuState == 1) {
			...
			if (eGm.linkSign.style.visibility=="visible" && eGc.link!= null && eGm.handleLinks && (evt.button!=eGm.showAltButton && eGm.showButton!=eGm.showAltButton || eGm.showButton==eGm.showAltButton) ) {// if a link is clicked without dragging and related option is checked
				...
				if (eGm.handleLinksAsOpenLink) {	 eGf.openLink(eGc.link);	}
				else {	// when option "use browser behavior" is checked to open links
					...
					if ( evt.button==1 ) {	// middle click
						...
						if (prefs.getBoolPref("tabs.opentabfor.middleclick") ) openNewTabWith(eGc.link);
						else openNewWindowWith(eGc.link);
					}
					else loadURI(eGc.link);	
				}
				eGm.close();
			}
			...
			if (evt.button==eGm.showAltButton && ( (eGm.menuState!=2 || eGm.menuState==2 && eGm.sector==-1) && eGm.showAltButton==eGm.showButton || eGm.showAltButton!=eGm.showButton ) ){
				...
				if (eGm.inputBox.style.visibility=="visible" ) {}
					...
					if ( eGm.sector!=-1 || eGm.sector==-1 && (eGm.menuState!=2 || eGm.menuState==2 && (evt.button!=eGm.showButton)) ) {
						...
						if ( ( actionName.search("searchWeb")!=-1 || actionName.search("translate")!=-1 || actionName.search("highlight")!=-1 ) && eGc.selection==""){ // Only for actions needing entry: searchWeb, translate, highlight
																															  // Selected text should not bring up input box
							if (actionName.search("searchWeb")!=-1) eGm.showPopupForSearchWeb(false);
                            eGm.showInputBox(false, actionName.search("highlight")!=-1 );	// second argument is to display options sign for highlight action
                            
						} else eGm.runAction();
					}
					else eGm.menuState = 3;
				}
            }
		}
}
...
function eG_handleMousemove(evt) {
	...
	if (eGm.typingText) { 
		if (eGm.inputBox.style.cursor!="auto") eGm.inputBox.style.cursor="auto";
		return false;
	}
	...
	if (eGm.menuState == 0) {
		...
		if (eGc.showAfterDelayPassed) return;
		
		if (eGm.showButton == 0 && eGm.dragOnlyUpLeft && eGm.dragOnly) eGc.draggedToOpen=((eGc.clientYDown-evt.clientY)> eGc.pieDragTolerance || (eGc.clientXDown-evt.clientX) > eGc.pieDragTolerance);
				
		if ( eGc.draggedToOpen && eGm.dragOnly){	// user dragged mouse to open menu
			
			// get selection if any
			if (eGc.selection == "" || eGc.selection==null) eGc.selection= eG_getSelection();				// save current selection before removal
		...
		if (eGc.draggedToOpen) {
			...
			if (!eGm.dropDownMenu && !eGm.showingTooltips) eGm.resetTooltipsTimeout(); 	// reset tooltips timeout
...
function eG_handleMousedown(evt)	{
	...
	if (eGm.menuState> 0 || eGm.autoscrollingState) {	//toggle primitive/alternative pie menu
	...
	for (var node = evt.originalTarget; node != null; node = node.parentNode){
        ...
        || evt.originalTarget.nodeName =="xul:scrollbarbutton" || evt.originalTarget.nodeName =="xul:thumb" || evt.originalTarget.nodeName =="xul:slider" ) {
            excludeTarget =true;
            break;
        }
	}
	...
	if ( excludeTarget || ( evt.button!= eGm.showButton ) || (eGc.keyPressed!= eGm.showKey && eGm.showKey!=0)
		|| (eGc.keyPressed == eGm.supprKey && eGm.supprKey!= 0)	)	{
		if (!eGm.dragOnly && excludeTarget) { eGc.blockStdContextMenu=true; }   // to be suppressed ?
		else {
			eGc.blockStdContextMenu = false;
			eGc.keyPressed = 0;
		}
	...
	eGc.contextType = "";
	if (eGc.link != null) eGc.contextType += "link/";
	if (eGc.image != null) eGc.contextType += "image/";
	if (eGc.contextType == "") {	// no need to go further if already link or image
		if (eGc.selection!=null && eGc.selection!="") eGc.contextType += "selection/";
		if (eGc.selectionNode!=null) eGc.contextType += "textbox/";
	}
	...
	if (eGm.autoscrollingOn) {// automatic delayed autoscrolling on mouse down
...
function eG_handleUnload(evt)	{
	...
	if (eGm.showButton ==2 && !eGc.allowContextBrowserOption && eGm.stateMenu==0 ) {	// "Disable or replace context menus" Browser option back to false if was false before showing pie menu
		eG_resetInitialContextBrowserOption();
	}
    ...
    if (needsChange == "needs-upgrade") eG_backupCustomIconsAndSkins();
    else if (needsChange == "needs-uninstall") eG_deleteAllPreferences();
}

function eG_countClicks(){	// update statsClicks preference
	try {	// because could be called from options.xul
		...
		if (eGm.menuState!=0) window.removeEventListener("mousedown", eG_countClicks, false);	
	} catch(ex) {}
}

function eG_resetInitialContextBrowserOption () {		// reset the initial value of 'Allow scripts to...Disable or replace context menu' in browser  options
...
function eG_openMenu() {
	...
	if (eGm.largeMenu && !eGm.dropDownMenu) {
		if (eGm.smallIcons) eGm.clientY += 9;
		else eGm.clientY += 15;
	}
	...
	if ( ( eGm.contextMenuAuto && eGc.contextType!="" && (eGc.keyPressed!= eGm.contextKey || eGm.contextKey==0) ) ||  ( !eGm.contextMenuAuto && eGc.contextType!="" && (eGc.keyPressed== eGm.contextKey) &&  eGm.contextKey!=0 ) ) {
		switch (eGc.contextType){
			case "link/":		eGm.show("contextLink"); break;
			case "image/":		eGm.show("contextImage");	break;
			case "link/image/":	if (eGm.contextImageFirst) eGm.show("contextImage"); else eGm.show("contextLink"); break;
			case "selection/":	eGm.show("contextSelection"); break;	
			case "textbox/":		eGm.show("contextTextbox"); break;
			case "selection/textbox/": if (eGm.contextTextboxFirst) eGm.show("contextTextbox"); else eGm.show("contextSelection");	break;
		}
...
function eG_showTextNotFoundStrip(phrase){
		...
		if (panel==null) panel =  getBrowser().mPanelContainer.firstChild;    // one tab only
...
function eG_showAlertStrip(evt, messageTitle, messageText){//  NO MORE USED

//************************* READ THIS **********************************
//      TO ACTIONS ICONS VISIBLE, ADD THE FOLLOWING CLASS DEFINITION LINE FOR EACH ACTION TO actions.css IN SKIN FOLDER
//      toolbarbutton.back { -moz-image-region: rect(128px 160px 160px 128px) !important ; }
//**********************************************************************
	
	// show dialog if menu can't be displayed
	if ( (evt.button == eGm.showButton) && (eGc.keyPressed!= eGm.supprKey && eGm.supprKey!= 0 || eGm.supprKey== 0)
	&& (eGc.keyPressed == eGm.showKey && eGm.showKey!=0  || eGm.showKey==0) ){
		
		if (document.getElementById("eGAlertMenu")==null)	{
		
			var panel =  getBrowser().mPanelContainer.selectedPanel;			//  many tabs
			if (panel==null) panel =  getBrowser().mPanelContainer.firstChild;	// one tab only
			
			hbox = document.createElementNS("http://www.mozilla.org/keymaster/gatekeeper/there.is.only.xul", "hbox");
			hbox.setAttribute("id","eGAlertMenu");
			hbox.setAttribute("style","background-color:#FFFFDD;border-bottom-style:inset;border-width:1px;");
			...
			message = document.createElementNS("http://www.mozilla.org/keymaster/gatekeeper/there.is.only.xul", "label");
			message.setAttribute("style","font-weight:bold;");
			message.setAttribute("value", messageTitle);
			vbox.appendChild(message);
			/*
			spacer= document.createElementNS("http://www.mozilla.org/keymaster/gatekeeper/there.is.only.xul", "spacer");
			spacer.setAttribute("flex","1");
			...
			*/
		    ...
		    /*
            // add here actions. The example below adds back action. DON'T FORGET to change css file as described at the begining
            toolbarbutton= document.createElementNS("http://www.mozilla.org/keymaster/gatekeeper/there.is.only.xul", "toolbarbutton");
			toolbarbutton.setAttribute("tooltiptext", eGc.localizing.getString("back"));
			if (getWebNavigation().canGoBack) {
				toolbarbutton.setAttribute("style","list-style-image: url('"+eGm.skinPath+"actions.png');");
				toolbarbutton.setAttribute("class","back");
				toolbarbutton.setAttribute("oncommand","this.parentNode.parentNode.removeChild(this.parentNode); eGf.back();");
				hbox.appendChild(toolbarbutton);
			}
          */  
			spacer= document.createElementNS("http://www.mozilla.org/keymaster/gatekeeper/there.is.only.xul", "spacer");
...
function eG_checkDailyReadingsFolder(){
     ...
     try {
        ...
        for (var i = 0; i < result.root.childCount; i++) {  // iterate over the immediate children of this folder
            if (result.root.getChild(i).itemId==eGm.dailyReadingsFolderURI && eGm.dailyReadingsFolderURI!="" || result.root.getChild(i).title==folderName) {
                ...
                if (eGm.dailyReadingsFolderURI=="") {   // update value if no value found
 	...
 	} catch(ex){ alert("Exception: "+ ex.toString()); return false;}
...
function eG_needsChange() { // returns "" if no change or "needs-upgrade" or "needs-uninstall"
    try {
        ...
        if (stagedFolder.exists()) { // if extensions\staged\{11F9F076-72B3-4586-995D-5042CF5D3AD4} folder exists, check if empty or not: empty means uninstall, not empty means update
            if (stagedFolder.fileSize!=0) return 'needs-upgrade';  // upgrading
            else return 'needs-uninstall'; //uninstalling
        }
        return "";
    } catch(ex) {return "";}
}

function eG_backupCustomIconsAndSkins() {    // save customicons and skins packs folders to a temp folder
...
function eG_restoreCustomIconsAndSkins() {   // restore customicons and skins packs folders from temp_ folder
    try {
        var fileLocator = Components.classes["@mozilla.org/file/directory_service;1"].getService(Components.interfaces.nsIProperties);				
        var userProfilePath =fileLocator.get("ProfD", Components.interfaces.nsIFile).path;
