...
var eGf = {
    ...
    all : function(start, end, showAll) {
        while(eGm.popup.hasChildNodes()){ eGm.popup.removeChild(eGm.popup.firstChild);}   // remove all from menupopup
		...
		for (var i=2; i<eG_menuItems.length; i++) {
            ...
            } else eGm.popup.appendChild(itemNode);
	...
	lastPage : function() {	getWebNavigation().gotoIndex(getWebNavigation().sessionHistory.count-1); 	 		},
	...
	forward : function() {	BrowserForward();		},
	
	back : function() {	BrowserBack();			},
	
	reload : function(loading)	{	// reload or stop
		if (!loading) BrowserReload();		//	BrowserReloadSkipCache();
		else BrowserStop();	
	},
	...
	up : function(url) {
		...
		for (var i=0;i<splitURL.length;i++)
			if (splitURL[i]!="" && splitURL[i]!=protocol){	host=splitURL[i];	break;	}
		//get directory
		...
		for (i=splitDIR.length-1;i>=0;i--){
			if (splitDIR[i]=="") {	
				if (updir!="") updir=updir.substring(0,updir.length-1);		//remove the slash at the end
			}
			else{	checkSubNetwork=false;	break;	}
		}
		
		// remove last part from url
		if (splitDIR[i]!="" && updir!="") updir =updir.substring(0,updir.length - splitDIR[i].length);
	...
	root : function(url, loadURL)	{	// when eG_canGoUp calls root, url must not be loaded
        ...
        var rootURL;
        try{rootURL = uri.scheme + "://"+ tld.getBaseDomainFromHost(uri.host)+"/";}catch(ex){rootURL = url;}    // do something when NS_ERROR_HOST_IS_IP_ADDRES or other exception is thrown
        if (loadURL) loadURI(rootURL);
        return rootURL;
	},
	
	pageTop : function() {var frame = eGc.evtMouseDown.originalTarget.ownerDocument.defaultView; frame.scroll(0,0);	},
	...
	prevTab : function() {
        ...
        if (eGm.tabPopupTimerId != null && eGm.tabPopupTimerFlag == true) {	// show popup
            ...
            for (var i=tabs.length-1;i>=0; i--) {               
                ...
                if (i== curTab) itemNode.setAttribute("disabled", true);
                if (i== curTab-1) itemNode.setAttribute("default", true);
                eGm.popup.appendChild(itemNode);
            }
            ...
            if(tabs.selectedIndex>0) tabs.advanceSelectedTab(-1);
            else tabs.selectedIndex=tabs.childNodes.length-1;
        }
	...
	nextTab	: function() { 
        ...
        if (eGm.showTooltips) clearTimeout(eGm.tooltipsTrigger);
            ...
            var tabs = getBrowser().mTabBox._tabs;
		    if (tabs.selectedIndex<tabs.childNodes.length-1) tabs.advanceSelectedTab(+1);
	...
	closeTab : function() {
		...
		if (gBrowser.tabContainer.childNodes.length > 1 || !eGm.closeBrowserOnLastTab) {
			if (gBrowser.tabContainer.childNodes.length==1) loadURI("about:blank");
			else gBrowser.removeCurrentTab();
		}
		else closeWindow(true); //BrowserCloseWindow() removed from FF3;
	},
	
	closeOtherTabs : function()	{ getBrowser().removeAllTabsBut( getBrowser().selectedTab); },
	undoCloseTab: function() {
        try { // to avoid error when Firefox pref "browser.sessionstore.enabled" is false
            ...
            for (var i=0; i<closedTabsArray.length; i++) {
                var itemNode = document.createElementNS("http://www.mozilla.org/keymaster/gatekeeper/there.is.only.xul", "menuitem");
                itemNode.setAttribute("class", "menuitem-iconic");
                if (i==0) itemNode.setAttribute("default", true);
	...
	closeOtherWindows : function() {
		...
		while (winEnum.hasMoreElements())	{	
			win = winEnum.getNext();
			if (winCur != win)	win.close();
		}		
	},
	
	closeBrowser : function()	{	goQuitApplication(); /*closeWindow(true);*/	},
	minimizeWindow : function()	{	window.minimize();	},
	fullscreen : function()	{
	
		BrowserFullScreen();
		//hide sidebar
		var sidebar = document.getElementById("sidebar");
		if (window.fullScreen && sidebar.hasAttribute("hidden") && !sidebar.hidden) toggleSidebar();
	},
	...
	openLink : function(link) {	
		var url;
		if (link==null) url = eG_readClipboard();
		else url = link.href;
		switch (eGm.openLink) {
			case "curTab":
				loadURI(url); break;
    ...
    sendLink : function(link) {
        if(link!=null) MailIntegration.sendMessage(link, this.root(link, false));
        else MailIntegration.sendMessage(eGc.doc.URL, eGc.doc.title);
	},
	...
	saveLinkAs : function(link) {	saveURL(link, null, null, false);	},
	
	saveImageAs : function(src, title) {	saveURL(src, title, null, false);	/*false means don't bypass cache*/	},	
	
	savePageAs : function(doc) { saveDocument(doc);	},	
	...
	dailyReadings : function() {
        ...
        if (dailyReadingsFolderNode!=null) {
            ...
            for (var i=0; i<bookmarksList.childCount; i++) {
                ...
                if(bookmarksList.getChild(i).icon!=null) {itemNode.setAttribute("src", bookmarksList.getChild(i).icon);}
                else itemNode.setAttribute("src", "" );
                itemNode.setAttribute("crop", "end");
                eGm.popup.appendChild(itemNode);
            }
	...
	searchWeb : function(string) {
        ...
        if (eGm.inputBox.style.visibility == "hidden" && eGm.popup.state !="showing" && eGm.popup.state !="hiding" && !eGm.inputBoxSignForSearchWeb.hasAttribute("noPopup")) {eGm.showPopupForSearchWeb(true);} // show popup before opening search textbox
        else {
            var first = true;
            var win = null;
            var chksearch;
            ...
            for (var i=1;i<=6;i++){
                ...
                if (query !="" && ( (chksearch && (currentEngine == 0) ) || (currentEngine == i)  ) ) {
                    ...
                    if (string!="") url = url.replace("%s",string);	
                    if (curURL!="") url = url.replace("%u",curURL);	
                    if (url==query) url = eG_getRoot(query);			// no placeholders to change the query
                    
                    url=encodeURI(url);
                    
                    if (!eGm.queryInNewWindow) {
                        if (!eGm.queryInNewTab && first) {loadURI(url); first=false;}
                        else	openNewTabWith(url); 
                        ...
                        if (first) { container._selectNewTab(tabs[tabs.length-1]); first=false;}  // selectNewTab removed from FF3
                    }
                    else{
                        if(win==null) {
                            win=window.open(url);
	...
	translate :function(string)	{
		...
		if (query !="") {
			...
			if (!eGm.resultQuery) {
				if (eGm.firstQuery && first) {loadURI(url);first=false;}
				else	openNewTabWith(url); 
	...
	runProgramFile : function(appNum, string) {//code adapted from "External Application Buttons" extension by Torisugari	
		...
		if (argumentstext != "") {
			if (string != "") argumentstext=argumentstext.replace("%s",string);
			var curURL = eGc.doc.URL;
			if (curURL != "") argumentstext=argumentstext.replace("%u",curURL);
		}
	...
	loadURLScript : function(appNum, string) {	
		...
		if (codetext != "") {
			if (string != "") codetext = codetext.replace("%s",string);
			var curURL = eGc.doc.URL;			// get current URL
			if (curURL != "") codetext = codetext.replace("%u",curURL);
		}
	...
	bookmarks : function() {	toggleSidebar("viewBookmarksSidebar");	},
	...
	history	: function() {	toggleSidebar("viewHistorySidebar");	},
	
	viewPageSource : function(doc) {	BrowserViewSourceOfDocument(doc);	},
    viewPageInfo : function(doc) {	BrowserPageInfo(doc, null);	},
    
	showOnlyThisFrame : function(frame_doc) { window.loadURI(frame_doc.location.href); },	
	
	properties : function(target) { 
        // no more available since FF 3.6: changed temporaly action to View Image Info
        if (target != null) BrowserPageInfo(target.ownerDocument, "mediaTab", target);
    },
	...
	selectAll : function() { 
		if (eGc.evtMouseDown.target instanceof HTMLInputElement || eGc.evtMouseDown.target instanceof HTMLTextAreaElement) eGc.evtMouseDown.target.focus();
		else window._content.focus();
	...
	highlight: function (phrase, win, changeColor)	{	// code adapted from "Googlebar" extension by GoogleBar Team
		...
		if (changeColor && phrase=="") {
			if (!confirm(eGc.localizing.getString("clearHighlightsConfirmation"))) return;
		}

		//this is to have same color in all frames
		if (changeColor) eGc.highlightColor = (eGc.highlightColor+1) % 10;
		...
		if (phrase=="") {
			...
			while ((elem = doc.getElementById("eGHighlight-id"))) {
				...
				while((child = elem.firstChild)) {
						if(child.id=="eGHighlightCount-id") break;		// removing count number after highlighted word 		
		...
		if (nodeSurround!=null && highlightCountOption) {
			if (nodeSurround.lastChild.innerHTML!=null){ nodeSurround.lastChild.innerHTML=countHighligths+"/"+countHighligths; }
	...
	zoomIn : function zoomIn() {
		...
		if (eGc.image==null) {  
			...
			if (eGc.image.style.width=="") width=eGc.image.width * 2 + "px";
			else width = parseInt(eGc.image.style.width) * 2 + "px";
			
			if (eGc.image.style.height=="") height=eGc.image.height * 2 + "px";
			else height = parseInt(eGc.image.style.height) * 2 + "px";
	...
	zoomOut : function() {
		...
		if (eGc.image==null) {  
			...
			if (eGc.image.style.width=="") width=eGc.image.width * 0.5 + "px";
			else width = parseInt(eGc.image.style.width) * 0.5 + "px";
			
			if (eGc.image.style.height=="") height=eGc.image.height * 0.5 + "px";
			else height = parseInt(eGc.image.style.height) * 0.5 + "px";
	...
	zoomReset :	function () { ZoomManager.reset();	}

}

//*********************************************************************************

function eG_getRoot(url) {	//this is a special code only for searchWeb and Translate functions: does not take into acount the subnetwork in a host

	while ( url.lastIndexOf("://")!=url.lastIndexOf("/")-2   )
	{	goodurl = url;	url= url.substring(0, url.substring(0, url.length-1).lastIndexOf('/')+1);	}
	return goodurl;
}
...
function eG_canGoUp () {
        ...
        var url = eGc.doc.URL;
        if (eGf.root(url, false) != url.replace("www.","")) return true;
        else return false;
}
...
function eG_xlink(node) {	// tag clicked link
	...
	// avoid tagging twice
	if ( (node.nextSibling instanceof HTMLImageElement) && node.nextSibling.src.indexOf("xLink.png") >=0) return;
